...
class ProductController extends \Controller
{
	...
	public function index()
	{
		...
		$result = $search
			...
			->sorting(Input::get('sortBy'), Input::get('directionBy'))
			->filterTypes(Input::get('types'))
			->filterEnabled(Input::get('enabled'))
			->filterDeleted(Input::get('deleted'))
            ->searchFieldPart('name', Input::get('name'))
            ->searchField('sid', Input::get('sid'))
            ->searchField('code', Input::get('code'))
			->paginate(30);
